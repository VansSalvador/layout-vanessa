...
(function($) { // Hide scope, no $ conflict
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
		...
		_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader) {
			...
			var selector = '<span class="omes ' + this._monthYearClass +
				'" title="' + inst.options.monthStatus + '">';
			for (var m = 1; m <= 12; m++) {
				if ((!minDate || plugin.newDate(year, m, plugin.daysInMonth(year, m)).
						getTime() >= minDate.getTime()) &&
						(!maxDate || plugin.newDate(year, m, 1).getTime() <= maxDate.getTime())) {
					selector += '<span value="' + m + '/' + year + '"' +
						(month === m ? ' class="selected"' : '') + '>' +
						monthNames[m - 1] + '</span>';
				}
			}
			selector += '</span>';
			...
			if (yearRange === 'any') {
				selector = '<span class="oano ' + this._monthYearClass + ' ' + this._anyYearClass +
				...
				selector = '<span class="oano ' + this._monthYearClass +
					'" title="' + inst.options.yearStatus + '">';
				start = plugin.add(plugin.newDate(start + 1, 1, 1), -1, 'd');
				end = plugin.newDate(end, 1, 1);
				var addYear = function(y, yDisplay) {
					if (y !== 0) {
						selector += '<span value="' + month + '/' + y + '"' +
							(year === y ? ' class="selected"' : '') + '>' + (yDisplay || y) + '</span>';
					}
				};
